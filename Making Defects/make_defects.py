...
def plot_images(images,columns=4,rows=4,cmap='jet',size=(12,12),Labels=[],images_name=""):
  fig, axes = plt.subplots(nrows=rows, ncols=columns,figsize=size)
  if Labels == []:
    Labels = ["Bulk","Vacancy","Anti Site","Ferro Electic"]
...
plot_images(images_to_plot,4,4,size=(12,12),images_name="Sr defects")

images_to_plot = np.concatenate((exp_clean[:4],TiVacancy_defect[:4],antiTi_defect[:4],TiFMag_defect[:4]))
plot_images(images_to_plot,4,4,size=(12,12),images_name="Ti defects")
